...
public class ClassGeneratorTest
{
    ...
    public void testClassGeneratorFromTableDefinition() throws Exception
    {
        ...
        for (Table table : schemaDao.getTables(schema.getSchemaName()))
        {
            ...
            List<? extends BaseObject> list = template.query("SELECT * FROM " + table.getTableName(), new ObjectMapper(
                    cls));
            assertNotNull(list);
    ...
    public void testGeneratorClassFromQueryResults() throws Exception
    {
        ...
        String query = "SELECT e1.*, d1.name as department_name FROM " + EnterpriseSchema.DEFAULT_SCHEMA + "."
                + EnterpriseSchema.TABLE_EMPLOYEE + " e1 INNER JOIN " + EnterpriseSchema.DEFAULT_SCHEMA + "."
                + EnterpriseSchema.TABLE_DEPARTMENT + " d1 ON e1.department_id = d1.id where d1.name = 'Bridge'";
    ...
    // {
    // String query =
    // "SELECT e1.*, d1.name as department_name FROM enterprise.employee e1 " +
    // "INNER JOIN enterprise.department d1 ON e1.department_id = d1.id where d1.name = 'Bridge'";
    //
    // JdbcTemplate template = new JdbcTemplate(testDatabase.getDataSource());
    // List<EnterpriseBridgeCrew> list = template.query(query, new
    // ObjectMapper<EnterpriseBridgeCrew>(EnterpriseBridgeCrew.class));
    // assertNotNull(list);
    //
    // assertEquals(EnterpriseSchema.getInstance().getBridgeCrewCount(),
    // list.size());
    // for (BaseObject obj : list)
    // {
    // assertTrue(obj.hasValues());
